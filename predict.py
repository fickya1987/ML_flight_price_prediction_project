...
def prediction_page():
    ...
    st.header('  Made by Mousumi Deb ')
    st.write(
        'Please fill bellow the details from the flight that you want to predict the ticket price')
    airline = st.selectbox(
        'Select Airline', ('Vistara', 'Air_India', 'Indigo', 'GO_FIRST', 'AirAsia'))
    source_city = st.selectbox(
        'Select departure city', ('Delhi', 'Mumbai', 'Bangalore', 'Kolkata', 'Hyderabad'))
    departure_time = st.selectbox(
        'Select departure time', ('Morning', 'Early_Morning', 'Evening', 'Night', 'Afternoon'))
    stops = st.selectbox('Select number of stops',
                         ('one', 'zero', 'two_or_more'))
    arrival_time = st.selectbox(
        'Select arrival time', ('Night', 'Evening', 'Morning', 'Afternoon', 'Early_Morning'))
    destination_city = st.selectbox(
        'Select destination city', ('Mumbai', 'Delhi', 'Bangalore', 'Kolkata', 'Hyderabad'))
    ...
    if ok:
        X = np.array([[airline, source_city, departure_time, stops,
                     arrival_time, destination_city, Class, days_left, duration]])
